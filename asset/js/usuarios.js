...
$("#formUsuarios").submit(function(event) {
    ...
    var host = $("#formUsuarios #host").val();
    var ip = $("#formUsuarios #ip").val();
    var port = $("#formUsuarios #port").val();
    var ubicacion = $("#formUsuarios #ubicacion").val();
    var tipo = $("#formUsuarios #tipo").val(); 
    var estado = $("input[name='estado']:checked").val(); // Captura el valor del radio button seleccionado
	var hosts_id = $("#formUsuarios #hosts_id").val(); 
        ...
        success: function(response) {
			if (submitType === "registrar") {
				if (response === "success") {
					...
					$("#formUsuarios #result").html("<div class='alert alert-danger text-center'>Error al registrar el hosts.</div>");
				}
			}else{
				if (response === "success") {
...
var eliminar_usuario_dataTable = function(tbody, table){
	...
	$(tbody).on("click", "button.table_eliminar", function(){
        ...
        eliminarUsuario(data.hosts_id, data.host);
	});
}

function eliminarUsuario(hosts_id, host){
	swal({
	  title: "¿Estas seguro?",
	...
	function(){
		deleteUser(hosts_id, host);
	});
}

function deleteUser(hosts_id, host) {
